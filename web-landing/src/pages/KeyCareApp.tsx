...
function ProductDemo() {
  ...
  useEffect(() => { setResult(null); setStatus('idle'); setError(''); setCopied(false); }, [text, action, targetLanguage]);
  const characterCount = useMemo(() => `${text.length}/2000`, [text.length]);
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!text.trim()) { setResult(null); setStatus('error'); setError(t('demo.emptyError')); return; }
    setResult(null); setCopied(false); setStatus('loading'); setError('');
    ...
    <section className="section demo-section" id="demo"><div className="shell">
      ...
      <div className="demo-shell">
        <form className="composer" onSubmit={submit}>
          ...
          <div className="composer-actions"><button className="button primary submit-button" type="submit" disabled={status === 'loading'}>{status === 'loading' ? t('demo.loading') : action === 'analyze' ? t('demo.analyzeButton') : t('demo.transformButton')}</button><button className="button secondary" type="button" disabled={status === 'loading' || (!text && !result)} onClick={clearDemo}>{t('demo.clear')}</button></div>
        ...
        <div className="result-panel" aria-live="polite" aria-busy={status === 'loading'}>
          ...
          {result && <div className="result-content">
            ...
            <div className="result-actions">{action !== 'analyze' && <button type="button" className="button primary" onClick={copyResult}>{copied ? t('demo.copied') : t('demo.copy')}</button>}<button type="button" className="button secondary" onClick={() => setResult(null)}>{t('demo.tryAnother')}</button></div>
          </div>}
        </div>
      </div>
    </div></section>
